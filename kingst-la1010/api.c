  ...
  for (i = 0; devlist[i]; i++)
    {
      ...
      if ((ret = libusb_open (devlist[i], &hdl)) < 0)
        {
          sr_warn(
              "Failed to open potential device with " "VID:PID %04x:%04x: %s.",
      ...
      else if ((ret = libusb_get_string_descriptor_ascii (
      ...
      if (des.iProduct == 0)
        {
          ...
          hdl, des.iProduct, (unsigned char *) product, sizeof(product))) < 0)
        {
      ...
      if (kingst_la1010_has_fx_firmware (hdl) == 0)
        {
          ...
          sr_dbg("Found an Kingst LA1010 device.");
...
static int
dev_clear (const struct sr_dev_driver *di)
{
...
dev_open (struct sr_dev_inst *sdi)
{
  ...
  if (devc->fw_updated > 0)
    {
      ...
      while (timediff_ms < MAX_RENUM_DELAY_MS)
        {
          if ((ret = kingst_la1010_dev_open (sdi, di)) == SR_OK)
            break;
          g_usleep (100 * 1000);
  ...
  else
    {
      ...
      ret = kingst_la1010_dev_open (sdi, di);
    }
  ...
  if (kingst_la1010_has_fx_firmware (usb->devhdl))
    {
      sr_err("Cyrus wasn't initialized.");
      return SR_ERR;
    }

  if (kingst_la1010_has_spartan_firmware (usb->devhdl))
    {
      sr_dbg("Spartan isn't initialized. Try to upload firmware.");
      if (kingst_la1010_upload_spartan_firmware (sdi))
        {
          sr_err("Upload Spartan firmware failed.");
          return SR_ERR;
        }
    }
  ...
  if (ret)
    {
      sr_err("Initialization of Spartan failed. Error: %s", libusb_error_name (ret));
      return SR_ERR;
    }
...
static int
config_get (uint32_t key, GVariant **data, const struct sr_dev_inst *sdi,
...
static int
config_set (uint32_t key, GVariant *data, const struct sr_dev_inst *sdi,
            const struct sr_channel_group *cg)
{
  struct dev_context *devc;
  struct sr_usb_dev_inst *usb;
...
static int
config_list (uint32_t key, GVariant **data, const struct sr_dev_inst *sdi,
             const struct sr_channel_group *cg)
{
  struct dev_context *devc;
...
static struct sr_dev_driver kingst_la1010_driver_info =
  {
      .name = "kingst-la1010",
      ...
      .dev_open = dev_open,
      .dev_close = dev_close,
      .dev_acquisition_start = kingst_la1010_start_acquisition,
